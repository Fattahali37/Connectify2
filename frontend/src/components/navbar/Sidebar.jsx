...
import { 
  exploreFill, homeFill, messageFill, messageOutline, 
  profileIcon, savedIcon, settingsIcon, switchAccountIcon, 
  exploreOutline, postUploadOutline, likeOutline, homeOutline 
} from "../../assets/svgIcons";
import Menu from '@mui/material/Menu';
import MenuItem from '@mui/material/MenuItem';
import Divider from '@mui/material/Divider';
import { Dialog, DialogContent, DialogTitle, TextField, Badge } from "@mui/material";
...
export const Sidebar = ({ active }) => {
    ...
    api.get(`${url}/user/notifications/unread-count`)
      .then((countRes) => {
        if (countRes?.data && countRes.data.success) {
          ...
          console.log('Refreshed notification count after closing:', countRes.data.count);
        }
      })
      .catch((err) => {
        console.error('Error refreshing notification count:', err?.message || err);
  ...
  useEffect(() => {
    console.log('Badge counts - Messages:', messageCount, 'Notifications:', notificationCount);
  ...
  return (
    <>
      <div className="sidebar-container">
        <div className="sidebar-content">
          ...
          <nav className="sidebar-nav">
            <NavLink to="/" className={`sidebar-item ${(active === "home" && !innerActive) ? 'active' : ''}`}>
            ...
            <NavLink 
              to="/explore" 
              className={`sidebar-item ${(active === "explore" && !innerActive) ? 'active' : ''}`}
            >
              {(active === "explore" && !innerActive) ? exploreFill : exploreOutline}
              <span>Explore</span>
            </NavLink>

            <NavLink 
              to="/chats/all" 
              className={`sidebar-item ${(active === "chat" && !innerActive) ? 'active' : ''}`}
            >
              <Badge 
                ...
                showZero={false}
              >
                {(active === "chat" && !innerActive) ? messageFill : messageOutline}
              </Badge>
              <span>Messages</span>
            </NavLink>

            <button 
              className={`sidebar-item sidebar-button ${innerActive === "notification" ? 'active' : ''}`}
              onClick={(event) => {
            ...
            >
              ...
              >
                {innerActive === "notification" ?
                  <svg aria-label="Notifications" className="sidebar-icon" color="#ffffff" fill="#ffffff" height="24" role="img" viewBox="0 0 48 48" width="24">
            ...
            <button 
              className={`sidebar-item sidebar-button ${innerActive === "newpost" ? 'active' : ''}`}
              onClick={handleClickOpen}
            >
              {innerActive === "newpost" ?
                <svg aria-label="New post" className="sidebar-icon" color="#ffffff" fill="#ffffff" height="24" role="img" viewBox="0 0 24 24" width="24">
            ...
            <NavLink 
              ...
              className={`sidebar-item ${(active === "myprofile" && !innerActive) ? 'active' : ''}`}
            >
              <img 
          ...
          <div className="sidebar-bottom">
            <button onClick={handleClick} className="sidebar-item sidebar-button">
              <svg aria-label="Settings" className="sidebar-icon" color="#ffffff" fill="#ffffff" height="24" role="img" viewBox="0 0 24 24" width="24">
      ...
      <Menu
        ...
        anchorOrigin={{ horizontal: 'right', vertical: 'bottom' }}
      >
        <MenuItem style={{ fontSize: '13px', fontFamily: 'Poppins', color: '#ffffff' }}>
          <Link to={`/${context.auth.username}`} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', width: '100%', color: '#ffffff' }}>
            {profileIcon}
            <span style={{ marginLeft: '12px' }}>Profile</span>
          </Link>
        </MenuItem>
        <MenuItem style={{ fontSize: '13px', fontFamily: 'Poppins', color: '#ffffff' }}>
          <Link to="/saved/thenisab" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', width: '100%', color: '#ffffff' }}>
            {savedIcon}
            <span style={{ marginLeft: '12px' }}>Saved</span>
          </Link>
        </MenuItem>
        <MenuItem style={{ fontSize: '13px', fontFamily: 'Poppins', color: '#ffffff' }}>
          <Link to="/accounts/edit" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', width: '100%', color: '#ffffff' }}>
            {settingsIcon}
        ...
        <MenuItem onClick={() => logout()} style={{ fontSize: '13px', fontFamily: 'Poppins', color: '#ffffff' }}>
      ...
      >
        <DialogTitle style={{ fontFamily: 'Poppins', textAlign: 'center', fontSize: '15.5px', color: '#ffffff' }} id="alert-dialog-title">
          {"Create new post"}
        </DialogTitle>
        ...
        <DialogContent>
          <div className="post" style={{ width: '45vw', height: '70vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', margin: 'auto' }}>
            {imgurl ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', width: '80%', position: 'absolute', bottom: 15, margin: 'auto' }}>
                  <TextField
                    ...
                    rows={4}
                    InputProps={{
                      style: { fontSize: '13.5px', fontFamily: 'Poppins', color: '#ffffff' }
                    }}
                    InputLabelProps={{
                      style: { color: '#aaa' }
                    }}
                  ...
                  />
                  <button onClick={() => handlePost()} style={{ border: 'none', outline: 'none', background: 'blue', padding: '8px 16px', borderRadius: '8px', color: 'white', backgroundColor: '#2196f3', marginTop: '12px', fontSize: '15px', cursor: 'pointer', fontWeight: 'bold' }}>
                    Upload
                  </button>
                </div>
              </div>
              ...
              <>
                <svg style={{ marginBottom: '10px', color: '#aaa' }} aria-label="Icon to represent media such as images or videos" fill="currentColor" height="77" role="img" viewBox="0 0 97.6 77.3" width="96">
                ...
                <p style={{ fontSize: '15px', color: '#aaa', marginBottom: '15px' }}>Drag photos and videos here</p>
                <label htmlFor="imgHandleUp" style={{ border: 'none', outline: 'none', padding: '8px 16px', borderRadius: '8px', color: 'white', backgroundColor: '#2196f3', fontSize: '15px', cursor: 'pointer', fontWeight: 'bold' }}>
